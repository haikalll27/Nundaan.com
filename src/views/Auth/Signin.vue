...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (!authError && data.user) {
      ...
      // 🔒 Gagal login, tangani berdasarkan status/message
      const status = authError.status;

      if (status === 400) {
        error.value = 'Permintaan tidak valid. Cek kembali data Anda.';
      } else if (status === 404) {
        error.value = 'Pengguna tidak ditemukan.';
      } else {
        error.value = 'Login gagal: ' + authError.message;
      }
    }

  } catch (err: any) {
    console.error('Unexpected Error:', err);
    error.value = 'Terjadi kesalahan server. Silakan coba lagi.';
  } finally {
    isLoading.value = false;
  }
};
